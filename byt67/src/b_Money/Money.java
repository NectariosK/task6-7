...
public class Money implements Comparable {
	...
	public Money add(Money other) {
		int addamount = this.getAmount() + other.getCurrency().valueInThisCurrency(other.getAmount(), this.getCurrency());
		return new Money(addamount, this.getCurrency());
	}
	...
	public Money sub(Money other) {
		Integer Sub = amount - currency.valueInThisCurrency(other.getAmount(), other.getCurrency());
		return new Money(Sub, currency);
	}
	...
	public int compareTo(Object other) {
		Money first_other= (Money) other;
		int first_currency=this.universalValue();
		int second_currency=first_other.universalValue();
		if(first_currency<second_currency)return -1;
		if(first_currency>second_currency)return 1;
		else return 0;
	}
}